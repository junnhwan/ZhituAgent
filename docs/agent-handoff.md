...
- 后端规模：`src/main/java` 当前约 177 个 Java 文件，`src/test/java` 当前约 66 个 Java 测试文件。
- 当前验证：
  - `.\mvnw.cmd -o test`：280 tests, 0 failures, 0 errors, 0 skipped。
  - `.\mvnw.cmd -o -Pbenchmark "-Dtest=MemorySummaryBenchmark" test`：1 benchmark test, 0 failures；结果写入 `target/memory-summary-bench/results-latest.md`。
...
| `src/main/java/com/zhituagent/memory` | Redis/in-memory 记忆、增量摘要状态、摘要压缩、事实抽取、压缩锁 |
...
- `ContextManager` 默认输入预算 1024 tokens，按旧消息、事实、摘要、evidence 的顺序降级；必要时会继续缩短保留的最小 recent window，仍超预算时打 `-overflow`。
- `MemoryService` 使用 `MemoryLock` 防止同一 session 并发压缩；Redis 开启时使用 Redis lock，否则 noop lock。
- 会话摘要默认仍走本地规则压缩；设置 `ZHITU_MEMORY_SUMMARY_ENABLED=true` 后启用增量 Markdown 摘要状态，Redis key 为 `zhitu:memory-summary:{sessionId}`，in-memory store 继续作为默认回退。
- LLM 摘要优先复用 `fallbackLlm`，例如通过 `zhitu.llm.router.fallback.model-name` 配置 mini 模型；mock mode 或摘要失败时不会持久化 mock 输出，而是走规则降级。
...
```powershell
.\mvnw.cmd -o -Pbenchmark "-Dtest=MemorySummaryBenchmark" test
Get-Content target\memory-summary-bench\results-latest.md
```
...
- 真实 mini 模型摘要需要显式配置 LLM 环境变量后单独验证；当前 benchmark 使用确定性 summarizer，不代表真实 provider 延迟或质量。
