...
精炼到"新 session Read 一次能 30 秒进入工作状态"。深度细节去看 `docs/optimize-progress.md` 与 user-level memory。
...
1. **`docs/optimize-progress.md`** — 项目主进度,完整 A-1..A-7 + 阶段 3 M1-M5 + v3-eval + 简历叙事框架最终版
...
- **路径使用规则**:默认使用项目真实路径 `D:\dev\my_proj\java\zhitu-agent-java\`。Bash 命令使用 unix-style 路径 `/d/dev/my_proj/java/zhitu-agent-java/`。Edit/Write 偶发 "File does not exist" 时,先重试一次 Read 再 Edit,仍失败才 fallback bash + heredoc/sed
