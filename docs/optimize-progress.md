...
1. **rerank ablation 是本次评测的真信号** — Qwen3-Reranker-8B 在固定 fixture 上贡献 **+5.6 pp nDCG@10 / +5.8 pp Recall@5**,代价是 p50 latency 3.8x(1.5s → 5.6s)
2. **明确的工程 trade-off** — 离线 / 高准确率开 rerank,实时 / 移动端 / 弱网关 rerank 换 4x 吞吐
3. **采样 fixture 的边界** — corpus 只 2000 docs(完整 T2Retrieval 是 118K),且 80% docs 对某 query 是 relevant,任务难度低于完整评测;**绝对值不能跟 MTEB leaderboard 直接比**,只能用同 fixture 内 ablation 的 Δ 做信号
...
- 落地 C-MTEB T2Retrieval 公开中文 retrieval benchmark 评测体系(BEIR 标准 qrels,采样 fixture: 2000 corpus / 300 queries / 1595 qrels),跳脱自造 query 集
- 跑通完整 retrieval-only eval pipeline(跳 LLM,~17 min 一组),指标含 nDCG@10 / Recall@k / MRR@k / Hit@k / p50-p99 latency
- **rerank ablation:同 fixture 上叠加 Qwen3-Reranker-8B 让 nDCG@10 从 0.86 → 0.91(+5.6 pp),但 p50 retrieve 从 1.5s → 5.6s(3.8x)**,据此落地 SLA-aware 检索决策
- 顺手补 zombie JVM defense(`System.exit` 强退)+ skipIngest flag(query-only 重跑复用 ES index)防御 sweep 死锁
- **诚实声明**:fixture 是采样的(corpus 只 2K vs 完整 118K),**绝对值不直接对标 MTEB leaderboard**,Δ 才是这次评测的真信号。完整 corpus 评测因 token 成本未做,留作 future work

---

## context-mgr-bench(2026-05-08)

`ContextManager` 默认值 bug fix + `@ConfigurationProperties` 配置化 + algorithm-layer micro benchmark 落地。详见 `docs/context-bench-results.md`。

### Phase A:bug fix

跑 `mvn -o test` 时发现 `BaselineEvalRunnerTest.shouldRunBaselineFixtureAndWriteEvalReport` pre-existing failure — `context-001` 期望 `contextStrategy="recent-summary"` 但拿到 `recent-summary-budgeted`。定位为 `ContextManager.maxInputTokens=640` 硬编码默认 < prod system prompt 实际 ~850 tokens(`chat-agent.txt`),**system prompt 单独就触发 budget loop**,所有 case 误标 `-budgeted`,trace 信号失真。

修复:新建 `ContextProperties`(`zhitu.context.*` prefix),5 个阈值都 config 化;默认 `maxInputTokens` 从 **640 → 1024**,留 ~170 token 头空间;`ContextManager` 加 `@Autowired(ContextProperties)` constructor。`mvn -o test` **258/258 全绿**,pre-existing failure 一并修复。

### Phase B:algorithm-layer benchmark

`ContextManagerBenchmark` (`@Tag("benchmark")`,`mvn -o -Pbenchmark test` 触发,~280 ms,产物 `target/context-bench/results-{ts}.{json,md}`):

| N(轮) | total messages | raw tokens | budgeted tokens | reduction | strategy |
|---|---|---|---|---|---|
| 10 | 20 | 1585 | 1010 | **36.3%** | recent-summary-facts-budgeted |
| 30 | 60 | 2879 | 1010 | 64.9% | 同上 |
| 50 | 100 | 4173 | 1010 | **75.8%** | 同上 |
| 100 | 200 | 7408 | 1010 | **86.4%** | 同上 |

形状:`raw O(N) 线性 vs budgeted ≈ 1010 tokens 收敛常数`。

### Phase C:真 LLM 端到端 ablation(待跑)

复用 `--zhitu.eval.compare-labels` 机制,跑两组 GLM-4 baseline 对比:`--zhitu.context.max-input-tokens=1024`(prod 默认)vs `=999999`(等价 disable budget),期望 routeAcc / Recall@5 / nDCG@5 / keywordCoverage ±2 pp 以内,但 averageInputTokenEstimate 减 60-80%。命令 + 占位表格在 `docs/context-bench-results.md` Phase C 段。

### 关键 insight

1. **ContextManager 默认值跟 prod 不匹配是个真 bug** — 简历讲得了 STAR(类似 A-7 fusion silent bug),不是空话
2. **algorithm-layer benchmark 跟 LangChain 自测同款方法** — 业界标准,不是 toy
3. **业界对标四件套**:LangChain `ConversationSummaryBufferMemory` + `ConversationEntityMemory` + Anthropic context engineering `compaction` / `context editing` / `Memory tool`
4. **诚实声明 token 数是 `TokenEstimator` 本地估算**,跟真 GLM/Qwen tokenizer 误差 ±15%,但 raw vs budgeted 的 ratio 不受影响
5. **algorithm-layer benchmark 是 sanity check,不是端到端质量评估** — 后者要 Phase C 真 LLM ablation 提供

### 简历叙事(context-mgr-bench 段)

- 落地 `ContextManager` 四级裁剪 + token 预算 + 摘要折叠 + 稳定事实抽取(对标 LangChain `ConversationSummaryBufferMemory` / `ConversationEntityMemory` + Anthropic context engineering)
- 通过 algorithm-layer micro benchmark 量化:50 轮 SRE 问答场景输入 token 减 75.8%,100 轮 -86.4%,raw O(N) vs budgeted 收敛常数
- 跑评测时拍出 `ContextManager.maxInputTokens=640` 硬编码 < prod system prompt 850 tokens 的默认值 bug,strategy 信号永远带 `-budgeted` 后缀失真;改 `@ConfigurationProperties` 驱动 + 默认 1024,ablation 时通过 `--zhitu.context.max-input-tokens=999999` 切关 budget 模式无需改代码
- (待 Phase C 完成)真 LLM ablation 显示 routeAcc / Recall@5 / nDCG@5 ±X pp 以内,token 节省不以质量损失为代价,对标 v3-eval rerank ablation 写法
